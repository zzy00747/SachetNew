...
/// 强智教务系统的用户信息 Provider
class QiangZhiUserProvider extends ChangeNotifier {
  ...
  Future init() async {
    String? name = await _secureStorageUtil.read(key: StoreItem.nameQZ.keyName);
    String? studentID =
        await _secureStorageUtil.read(key: StoreItem.studentIDQZ.keyName);
    String? cookie =
        await _secureStorageUtil.read(key: StoreItem.cookieQZ.keyName);
    ...
    if (newUser.cookie != _user.cookie) {
      ...
      if (_user.cookie != null) {
        await _secureStorageUtil.save(
          key: StoreItem.cookieQZ.keyName,
          value: _user.cookie,
        );
      }
      if (_user.name != null) {
        await _secureStorageUtil.save(
          key: StoreItem.nameQZ.keyName,
          value: _user.name,
        );
      }
      if (_user.studentID != null) {
        await _secureStorageUtil.save(
          key: StoreItem.studentIDQZ.keyName,
  ...
  static Future<String?> readPassword() async {
    return _secureStorageUtil.read(key: StoreItem.passwordQZ.keyName);
  }

  /// 保存密码
  static Future savePassword(String password) async {
    await _secureStorageUtil.save(
      key: StoreItem.passwordQZ.keyName,
      value: password,
    );
  }

...
import 'fetch_grade_semesters_and_alert_text.dart';
import 'parse_grade_semesters_and_alert_text.dart';
...
Future<
  ...
  try {
    final html = await fetchGradeSemetersAndAlertTextZF(cookie: cookie);
    ...
    if (e == '获取可查询学期数据失败: Http status code = 302, 可能需要重新登录') {
      ...
      // 重新加载数据
      final html = await fetchGradeSemetersAndAlertTextZF(cookie: newCookie);
      return parseGradeSemestersAndDangerTextFromHtmlZF(html);
    } else {
      rethrow;
    }
